...
import { FriendshipConnection, FriendshipConnectionArgs } from '../friendship/FriendshipType';
import { FriendshipFilterInputType } from '../friendship/FriendshipFilterInputType';
...
const UserType = new GraphQLObjectType<UserDocument, GraphQLContext>({
  ...
  fields: () => ({
    ...
    friendships: {
      type: FriendshipConnection.connectionType,
      args: {
        input: {
    ...
    isFriend: {
      ...
      resolve: async (user, __, ctx) => {
        ...
        const friendship = await FriendshipModel.findOne({ sender: ctx.user.id, receiver: user.id });
        return !!friendship;
      },
    },
    ...objectIdResolver,
