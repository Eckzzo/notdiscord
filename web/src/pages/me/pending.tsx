...
import pageQuery, { pending_PageQuery } from '__generated__/pending_PageQuery.graphql';
import { FriendshipPendingList } from 'components/Friendship/FriendshipPendingList';

const PendingQuery = graphql`
  query pending_PageQuery($after: String, $first: Int!, $status: FriendshipStatus!, $target: FriendshipTarget!) {
...
const Pending: NextPageWithLayout<PendingProps> = ({ queryRefs }: PendingProps) => {
  const data = usePreloadedQuery<pending_PageQuery>(PendingQuery, queryRefs.pageQueryRef);
