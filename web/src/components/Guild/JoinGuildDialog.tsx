...
import { GuildJoinMutation, GuildJoinMutation$data } from '__generated__/GuildJoinMutation.graphql';
...
const JoinGuildDialog: React.FC<JoinGuildDialogProps> = ({ children, fragmentKey }) => {
  const [joinGuild] = useMutation<GuildJoinMutation>(GuildJoin);
  ...
  const onSubmit = (input: FormValues) => {
    const config = {
      ...
      onCompleted: (data: GuildJoinMutation$data) => {
        if (!data.GuildJoinMutation) return;
