...
const FriendshipSentListFragment = graphql`
  fragment FriendshipSentListFragment on Query @refetchable(queryName: "FriendshipsSent") {
...
const FriendshipSentList: React.FC<FriendshipSentListProps> = ({ fragmentRef }) => {
  const [cancelFriendship, isLoading] = useMutation<FriendshipCancelMutation>(FriendshipCancel);

  const { data } = usePaginationFragment(FriendshipSentListFragment, fragmentRef);
    ...
    return (
      <Flex align='center' justify='center' direction='column' gap={4} grow>
        <Flex direction='column' align='center' gap={2}>
          ...
          <Text color='lowContrast'>We don&apos;t have a Wumpus, but maybe you can...</Text>
        </Flex>
        <AddFriendDialog>
        ...
        if (edge?.node?.recipient) {
          return (
            <Fragment key={edge?.node?.id}>
              <FriendshipCard message='Outgoing Friend Request' fragmentRef={edge?.node?.recipient}>
                <Tooltip content='Cancel' side='top' sideOffset={4}>
                  <IconButton
                    variant='tertiary'
                    onClick={() => {
                      if (edge && edge.node) handleCancel(edge.node.id);
                    }}
                  >
                    {isLoading ? <Spinner /> : <Cross2Icon height={20} width={20} />}
                  </IconButton>
                </Tooltip>
              </FriendshipCard>
              <Separator />
