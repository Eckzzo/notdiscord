...
const MessageForm: React.FC<MessageFormProps> = ({ fragmentKey, location }) => {
  const [createMessage, isLoading] = useMutation<MessageCreateMutation>(MessageCreate);
    ...
    <Flex css={{ p: '$4', borderTop: '1px solid $gray300' }}>
      <Form onSubmit={handleSubmit(onSubmit)} css={{ width: '100%' }}>
        <Field>
          ...
          <Field.Input variant='secondary' placeholder='Type a message' {...register('content')} />
        </Field>
      </Form>
    </Flex>
  );
